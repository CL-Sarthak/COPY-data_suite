...
export async function POST(
  ...
  try {
    ...
    const body = await request.json() as QueryImportRequest;
    const { query, name, description } = body;
    ...
    try {
      ...
      const queryPromise = connector.executeQuery(query);
      
      const result = await Promise.race([queryPromise, timeoutPromise]);
      const executionTime = Date.now() - startTime;

      // Convert query result to JSON format for data source
      const jsonData = result.rows.map((row: unknown[]) => {
      ...
      // Create data source from query results
      const dataSource = await DataSourceService.createDataSource({
        ...
        configuration: {
          ...
          description: description || `Query import from ${connection.name} - Query: ${query}`,
          data: jsonData,
          files: [{
            name: `${name}.json`,
    ...
    logger.error('Query import error:', error);
    
    const errorMessage = error instanceof Error ? error.message : 'Query import failed';
